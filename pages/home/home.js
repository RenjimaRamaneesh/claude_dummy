...
Page({
  ...
  onLoad: function () {
    ...
    // Fixed elements: status bar + header(~50) + search(~45) + carousel(~190) + filter tabs(~50) + bottom nav(~55)
    var fixedHeight = statusBarHeight + 50 + 45 + carouselHeight + 50 + 55;
    var scrollHeight = screenHeight - fixedHeight;
  ...
  onContentScroll: function (e) {
    ...
    this._scrollTimer = setTimeout(function () {
      if (diff > 5 && !self.data.carouselCollapsed && scrollTop > 20) {
        // Scrolling down - collapse carousel, expand scroll area
        var carouselHeight = self.data.carouselHeight;
        self.setData({
          carouselCollapsed: true,
          scrollHeight: self.data.scrollHeight + carouselHeight
        });
      } else if (diff < -5 && self.data.carouselCollapsed && scrollTop < 10) {
        // Scrolling up near top - expand carousel, shrink scroll area
        var carouselHeight = self.data.carouselHeight;
        self.setData({
          carouselCollapsed: false,
          scrollHeight: self.data.scrollHeight - carouselHeight
  ...
  // Bottom Navigation - use redirectTo for tab switching
  onNavTap: function (e) {
